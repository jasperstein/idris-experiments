...
parseCommand "search" term = Just (Search term)
...
searchItems : Nat -> String -> Vect n String -> String
searchItems _ _  [] = ""
searchItems k term (y :: xs) = let rest = searchItems (S k) term xs in
    if isInfixOf term y 
    then ((show k) ++ ": " ++ y ++ "\n" ++ rest)
    else rest

search : String -> DataStore -> String
search term store = searchItems 0 term (items store)
