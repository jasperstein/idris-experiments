...
splitInput input = let s = split (== '-') input in
	case (init' s, last' s) of
		(Just inits, Just lastPart) => (concat inits, lastPart)

toSectorAndCheckSum : String -> (Integer, List Char)
toSectorAndCheckSum s = let (sec, chk) = span (/= '[') s in
	case (parseInteger sec) of Just sector => (sector, filter isLower $ unpack chk)

startCounts : List (Char, Nat)
startCounts = zip ['a'..'z'] (replicate 26 Z)

increaseCount : Char -> List (Char, Nat) -> List (Char, Nat)
increaseCount c counts = map (\(ch, nm) => (ch, if ch == c then (S nm) else nm)) counts

calculateFreqs : String -> List (Char, Nat)
calculateFreqs s = let cs = unpack s in
		foldl (\cnts => \c => increaseCount c cnts) startCounts cs

reversedOrder : Ordering -> Ordering
reversedOrder LT = GT
reversedOrder EQ = EQ
reversedOrder GT = LT

sortedFreqs : List (Char, Nat) -> List (Char, Nat)
sortedFreqs fs = sortBy (\(c1, f1) => \(c2, f2) => 
	(thenCompare (reversedOrder $ compare f1 f2) (compare c1 c2))) fs

calculateChecksum : String -> List Char
calculateChecksum s = (map Prelude.Basics.fst) . (take 5) . sortedFreqs . calculateFreqs $ s

sectorOfRealRoom : String -> Maybe Integer
sectorOfRealRoom input = let (name, output) = splitInput input in
	let (sector, checksum) = toSectorAndCheckSum output in
	let calculated = calculateChecksum name in
		if calculated == checksum then Just sector else Nothing
